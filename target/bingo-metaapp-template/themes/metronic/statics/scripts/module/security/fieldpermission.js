    ...
    return {
    	createNew:function(params)
    	{
	    	...
	    	var turnOffPermission=function (sender,permissionId){
		       	$.messageBox.confirm({message:i18n.t("permission.turnoffAlert"),
					callback:function(result){
						if(result){
							var url=Global.contextPath+"/sec/entities/turnoff_fieldpermission";
							url=Urls.appendParam(url,"fieldId",permissionId);
			...
			var turnOnPermission=function(sender,permissionId){
				var url=Global.contextPath+"/sec/entities/turnon_fieldpermission";
		       	url=Urls.appendParam(url,"fieldId",permissionId);
		       	url=Urls.appendDate(url);
		       	jQuery.restPost(url,null,function(response){
		       		 if(response){
			...
			var addPermission=function(sender){
				...
				var permissionId=tbodyEl.attr("id")+":"+tdEl.attr("data-operation");
			...
			var buildInControlTd=function(role,hasPermission){
				...
				html+="<td data-operation='Select' style='text-align: center;'><a class='role-permission'  data-status='"+dataStatus+"'><img src='"+permissionIcon+"'/></a></td>";
				html+="<td data-operation='Insert' style='text-align: center;'><a class='role-permission'  data-status='"+dataStatus+"'><img src='"+permissionIcon+"'/></a></td>";
				html+="<td data-operation='Update' style='text-align: center;'><a class='role-permission'  data-status='"+dataStatus+"'><img src='"+permissionIcon+"'/></a></td>";
				html+="<td></td>";
				return html;
			}
