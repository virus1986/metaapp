...
(function ($) {
    ...
    $.htmlClean = function (html, options) {
        ...
        while (tagMatch = tagsRE.exec(html)) {
            var tag = tagMatch[6]
                ...
                if($.inArray(tagMatch[3],options.includeNamespaces) > -1){
                	tag=new Tag(tagMatch[3]+tagMatch[4], tagMatch[2], tagMatch[5], options);
                }
            ...
            if (tag.isClosing) {
                ...
                while (attrMatch = attrsRE.exec(tag.rawAttributes)) {
                    ...
                    if (tag.allowedAttributes != null
                    	...
                    	var _attr=new Attribute(attrMatch[1], attrMatch[2]);
                    	if(attrMatch[1].indexOf("_")===0){
                    		var _newName=attrMatch[1];
                			if(attrMatch[1].indexOf("_")===0){
                				_newName=attrMatch[1].substring(1);
                			}
                    		_attr=new Attribute(_newName, attrMatch[2]);
                    	}
                        element.attributes.push(_attr);
                    }
                    if(tag.allowedAttrPrefix!=null&&tag.allowedAttrPrefix.length>0){
                    	for(var i=0;i<tag.allowedAttrPrefix.length;++i){
                    		if(attrMatch[1].indexOf(tag.allowedAttrPrefix[i])===0){
                    			var _newName=attrMatch[1];
                    			if(attrMatch[1].indexOf("_")===0){
                    				_newName=attrMatch[1].substring(1);
                    			}
            ...
            if (!element.isRoot && renderTag) {
                ...
                $.each(element.attributes, function () {
                    if ($.inArray(this.name, options.removeAttrs) == -1) {
                        ...
                        if (this.name == "class" && options.disAllowedClasses.length > 0) {
                        	value= $.grep(value.split(" "), function (c) {
                        		for(var _i=0;_i<options.disAllowedClasses.length;++_i){
                        			var a=options.disAllowedClasses[_i];
                        			if(c==a || (a[0] == c && (a.length == 1 || $.inArray(element.tag.name, a[1]) > -1))){
                        				return false;
                        			}
                        		}
                        		return true;
    ...
    function Tag(name, close, rawAttributes, options) {
        ...
        this.init = function () {
            ...
            if (options) {
                ...
                this.allowedAttrPrefix=options.allowedAttrPrefix;
            }
        }
    ...
    var tagAllowEmpty = ["th", "td","span","label"];
    var tagRequiredParent = [
        null,
        "li", ["ul", "ol","meta:relation"],
        "dt", ["dl"],
        "dd", ["dl"],
        "td", ["tr"],
        "th", ["tr"],
