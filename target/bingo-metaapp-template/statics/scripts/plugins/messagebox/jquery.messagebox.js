...
(function($) {
	var topWin = window ;
	var $config=topWin.$messageboxConfig||{
		okButton:"&nbsp;OK&nbsp;",
		cancelButton: '&nbsp;Cancel&nbsp;',
		info: "Information",
		warning: "Warning!",
	...
	$.messageBox = {
		...
		okButton: $config.okButton,         // text for the OK button
		cancelButton: $config.cancelButton, // text for the Cancel button
		...
		_show: function(option) {
			...
			var okButton = option.okButton||this.okButton;
			...
			if(option.okButton != null && option.okButton != '' && option.okButton != undefined){
				if(option.okButton.length > 6){
					alert('button name should less than 6 characters');
					return ;
				}
				okButton = option.okButton;
			}
			
			var cancelButton = option.cancelButton||this.cancelButton;
			...
			$("BODY").append(
			  '<div id="popup_container" class="ui-widget ui-widget-content ui-corner-all">' +
			    ...
			    '<div id="popup_content' + (type?("_"+type):"") + '">' +
			...
			//$("#popup_message").html(message.replace(/\n/g, '<br />'));
			function encodeHtml(rawHtml){
				if(!rawHtml){
					return rawHtml;
				}
				return $("<div></div>").text(rawHtml)[0].innerHTML;
			};
			$("#popup_message").html(encodeHtml(message));
			...
			switch( type ) {
				case 'confirm':
					$("#popup_message").after('<div id="popup_panel"><button class="btn" type="button" id="popup_cancel">' + cancelButton + '</button> <button type="button" class="btn  blue" id="popup_ok">' + okButton + '</button></div>');
				...
				break;
				case 'prompt':
					$("#popup_message").append('<br /><input type="' + (option.password?'password':'text') + '" size="30" id="popup_prompt" />').after('<div id="popup_panel"><button type="button" class="btn" id="popup_cancel">' + cancelButton + '</button> <button type="button" class="btn  blue" id="popup_ok">' + okButton + '</button></div>');
