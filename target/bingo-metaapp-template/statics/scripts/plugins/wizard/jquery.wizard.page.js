	...
	*/
	...
	Wizard.prototype.execEvent = function(eventName , activeStep ,type ){
		...
		if( toStepName === false ){
			...
			this.block({ message:'Page jumping...' }); 
	...
	Wizard.prototype.drawControlBar = function () {
		...
		if(lastIndex != undefined && lastIndex != null && lastIndex != ''){
			$("<button id='last' class='btn btn-primary'><i class='icon-chevron-left icon-white'></i>Prev</button>")
		...
		var nextStepText = this.getActiveNode().nextStepText||'Next' ;
		...
		if(this.getActiveNode().canComplete == 'yes' || nextIndex == undefined){//完成按钮
			$("<button id='complete' class='btn btn-primary'><i class='icon-ok icon-white'></i>Complete</button>")
				.appendTo(".wizard-control") 
				.click(function(){
					wizardObj.complete();
					return false;
		...
		$("<button id='close' class='btn hide'><i class='icon-remove'></i>Close</button>")
			.appendTo(".wizard-control") 
			.click(function(){
				wizardObj.cancel();
				$(document.body).dialogClose() ;
	...
	Wizard.prototype.hideStep = function (name, force) {
		...
		if(!force && name == this.currentIndexId){
			alert('Can not hide the current step');
			return false;
		}else{
		...
		if(!node.nextStepName){
			alert('Can not hide the last step');
			return false;
		}
		if(!node.prevStepName){
			alert('Can not hide the first step');
			return false;
		}
		node.hide = true;
		this.drawFlowBar();
