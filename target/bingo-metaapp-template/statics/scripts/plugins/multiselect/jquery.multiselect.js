...
;(function($){
	if(jQuery) (function(jQuery){
		...
		var clazz = {
			...
			checked:'checked ui-state-active  multi-checked',
			...
			__multiSelect: function(o, callback) {
				...
				jQuery(this).each( function() {
					...
					if( select.next().hasClass("multi-select")||select.next().hasClass("multiSelect-cont") ){
						...
						html+= '<span  class="add-on btn button-reset"><i class="icon-chevron-down"></i></span>' ;
						html += '</span>';
						select.after(html);
					}
					...
					var _multiSelectOptions = $(html).appendTo(select.parent()).css({top:"0px",left:"0px"}).hide() ;
							...
							if( i == 0 ) {
								...
								if( o.oneOrMoreSelected == '*' ) {
									...
									_multiSelectOptions.find('INPUT:checkbox:checked').each( function() {
										var self=$(this);
										var text=self.parent().text()||self.closest("label").text();
										if( text != o.selectAllText ) {
											display.push(jQuery.trim(text)) ;
										}
