		...
		newLang : function() {
			$.validationConfig.allRules = {
				...
				"name":{
					"regex" : /^[a-zA-Z0-9_]*$/,
					"executor" : "_customRegex",
					"alertText" : "字母下划线开头，允许数字字母下划线"
				},
				"id":{
					"regex" : /^[a-zA-Z_][a-zA-Z0-9_]*$/,
					"executor" : "_customRegex",
					"alertText" : "只允许字母数字下划线"
				},
				"letterStart":{
					"regex" : /^[a-zA-Z][a-zA-Z0-9_]*$/,
					"executor" : "_customRegex",
					"alertText" : "字母开头，只允许字母数字下划线"
				}
			}
		}
	}
...
(function($) {
	...
	$.fn.validation = function(settings) {
		...
		if (settings.inlineValidation == true) { // Validating Inline ?
			if (!settings.returnIsValid) { // NEEDED FOR THE SETTING returnIsValid
				...
				$(this).find(validtorAttr).focusin(function(){
					var caller=$(this);
		...
		//required
		if(settings.showRequiredStar){
			settings.labelClass=settings.labelClass||"control-group";
			$(this).find(validtorAttr).each(function(){
				if( $(this).attr(validtorAttrValue).indexOf("required")!=-1 ){
					...
					}else if($(this).parents("td:first").length>0){
					...
					}else{//bootstrap form layout
						var con=$(this).closest(".form-group");
						if(con.length===1){
							var label=con.find("label:eq(0)");
							if(label.find(".required-star").length===0){
								label.prepend("<span class='required-star'>*</span>") 
							}
						}else{
							return;
						}
	...
	$.validation = {
		...
		validateCall : function(caller, rules,eventType) { // EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
			...
			$.each(rules, function(i, v) {
				...
				if (validator) {
                    ...
                    $.validation.debug("验证器拼写有误: " + "name=" +$(caller).attr("id") + " validator name=" + this.name);
                    return false;
                }
			...
			function _funcCall(caller, rule,eventType) { // VALIDATE CUSTOM FUNCTIONS OUTSIDE OF THE ENGINE SCOPE
				...
				try{
					...
					console.log("validator function not exists:"+funce);
				}
				if (typeof(fn) === 'function') {
					//var fn_result = fn(caller,eventType);
					var params=[caller,eventType];
					if(rule.options.length>1){
						params=params.concat(rule.options.slice(1));
					}
					var fn_result = fn.apply(caller,params);
			...
			function _length(caller, rule) { // VALIDATE LENGTH
				...
				if (feildLength < minL || feildLength > maxL) {
					$.validation.isError = true;
					promptText += _buildPromptText("当前输入长度为"+feildLength+"[输入长度必须在" + minL + "和" + maxL + "之间,1个汉字长度为2]", rule.options[2]);
				}
			}
		...
		buildPrompt : function(caller, promptText) { // ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
			...
			if( $(caller).parents("._td_layout")[0] || !$(caller).parents(".error")[0]   ){
					/**
					 * 2014-7-16
					 * td:first 去掉 td
					 * 只改第一个
					 */
					$(caller).attr("errorInfo", promptText).parents(":first").removeClass("success").addClass("_td_layout error");//.append("<span class='tipsy-container tipsy-error'></span>")  ;
					$(caller).attr("errorInfo", promptText).parents("td:first").prev().find("label").addClass("validator-error-label") ;
			}else{
					$(caller).parents(".error:first").removeClass("error success") ;
				    $(caller).attr("errorInfo", promptText).parents(".error:first").addClass("error").find(">label").addClass("validator-error-label");
			}
		},
		...
		closePrompt : function(caller) { // CLOSE PROMPT WHEN ERROR CORRECTED
			...
			/*
			    2014-7-15
			             去除了success、validator-success-label这两个样式。
			             即通过校验不展示效果
			 */
			if( $(caller).parents("._td_layout")[0] || !$(caller).parents(".error")[0]   ){
				/**
				 ...
				 * 只改第一个
				 */
				$(caller).parents(":first").removeClass("error").addClass("_td_layout");//.append("<span class='tipsy-container tipsy-success'></span>") ;
				$(caller).removeAttr("errorInfo").parents("td:first").prev().find("label").removeClass("validator-error-label") ;
			}else{
				$(caller).parents("._td_layout:first").removeClass("error success") ;
