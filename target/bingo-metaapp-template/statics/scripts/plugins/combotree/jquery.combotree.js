...
(function($){
	function setSize(target){
		...
		var __appendText =  '<a  class="clear-content" title="Clear" style="cursor:pointer;">[Clear]</a>' ;

		var __tree = content.find('>ul').tree($.extend({//tree为容器ID
			method : 'post',
			asyn : true, //异步
			rootId  : 'root',
	...
	function init(target,options){
		...
		if( $(target).next().hasClass("combotree") ){
			...
			arrow = $('<span class="add-on btn button-reset">选择<i class="icon-angle-down"></i></span>').appendTo(span);
		}
		...
		content = $('<div class="ui-helper-reset ui-widget-content ui-corner-all combotree-content"><ul></ul></div>').appendTo(document.body).css({top:"0px",left:"0px",position:"fixed"}).hide();//.appendTo(span);
