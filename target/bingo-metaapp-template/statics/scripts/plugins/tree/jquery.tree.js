...
(function($) {
    ...
    $.fn.lighttree = function(settings,itemOrItemId,level,asyn) {
        ...
        var dfop =
            {
                ...
                root:{value:'root',text:'Root',icon:'bbit-tree-root'},
                data: null,
                clicktoggle: true, //点击节点展开和收缩子节点
                theme: "bbit-tree-arrows", //bbit-tree-lines ,bbit-tree-no-lines,bbit-tree-arrows 
                //定义默认图片地址 tree-toggle
               // icons:{floderOpen:"bbit-tree-folderopen",leaf:"bbit-tree-leaf",floder:"bbit-tree-folder",loading:'bbit-tree-loading'},
                icons:{floderOpen:"tree-toggle",leaf:"bbit-tree-leaf",floder:"tree-toggle closed",loading:'bbit-tree-loading'},
                params : {},
                rootId:"root",
                rootText : false,
                isRootExpand:true,
        ...
        function nodeClick(e) {
            ...
            if (et.tagName == "DIV" && !$(et).hasClass("bbit-tree-node-el")) {
            ...
            }else if(et.tagName == 'SPAN'||(et.tagName == "DIV" && $(et).hasClass("bbit-tree-node-el"))){
                if (dfop.citem){
        ...
        function asnyloadc(pnode, isAsync, callback) {
            ...
            if ( url ) {
				...
				$.dataservice(dfop.CommandName,service_param, callback ,{async:false,url:url}) ;
            }
        }
        ...
        function bindevent() {
            $(this).hover(function() {
             ...
             if(dfop.contextMenu){//如果存在右键菜单
            	 var __eventType=dfop.contextMenuEventType||"contextmenu";
            	 $(this).bind(__eventType, function(e){
            		 var et = e.target || e.srcElement;
                     if (et.tagName == "DIV"&&$(et).hasClass("bbit-tree-node-el")) {
	    ...
	    if(!options){
        	...
        	options.eventType = options.eventType||'contextmenu' ;
        ...
        me[0].t = {
            ...
            collapse :function(item){
            	...
            	disabled(item, bool) ;
            },upMoveNode:function(item){//同级节点向上移动
            	if(item){
            		
            		var nodeid=item.id;
            		var parentId=item.parentId;
            		var parentLi=$("[nodeid='"+nodeid+"']",me).parent();
            		var prev=parentLi.prev("li");
            		if(prev.length>0){
            			parentLi.after(prev);
            			var sn = searchNode(parentId) ;
            			if(sn){
            				var childNodes= sn.node.childNodes||[] ;
            				for(var i=0;i<childNodes.length;++i){
            					var current=childNodes[i];
            					if(current.id==nodeid){//找到当前交换的节点
            						var up=childNodes[i-1];
            						childNodes[i-1]=current;
            						childNodes[i]=up;
            						sn.node.childNodes=childNodes;
            						break;
            					}
            				}
            			}
            ...
            },downMoveNode:function(item){//同级节点向下移动
            	if(item){
            		var nodeid=item.id;
            		var parentId=item.parentId;
            		var parentLi=$("[nodeid='"+nodeid+"']",me).parent();
    ...
    $.extend($.fn , {
	    ...
	    },upMoveNode:function(item){
	    	if (this[0].t) {
	            return this[0].t.upMoveNode(item);
	        }
	    },downMoveNode:function(item){
	    	if (this[0].t) {
	            return this[0].t.downMoveNode(item);
	        }
	    }
    }) ;
    ...
    $.fn.tree = function(json_obj){
    	if( !this.length ){
    		alert("Init error. Selector["+this.selector +"] not exists, please check the spelling!") ;
    		return ;
    	}
	...
	treeWidget = function(){
		...
		var events = ['getAllNodes','getSelectedIds','getSelectNodes','getCurrentNode',
		              'refresh','expandAll','collapse','expandLevel','isLeaf','treeOption',
		              'deleteNode','updateNode','addNode','checkAll','checkNode','disableNode','upMoveNode','downMoveNode'] ;
