				...
				if(ts.p.cellEdit === true) {
					...
					if(ts.p.multiselect && ts.p.multiboxonly) {
						if(scb){$(ts).jqGrid("setSelection",ri,true,e);}
							...
							ts.p.selarrrow = [];
							$(ts).jqGrid("setSelection",ri,true,e,true);//fixed by lijing:added reset param to setselection
						}
					} else {
						$(ts).jqGrid("setSelection",ri,true,e);
					}
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr, e,reset) {
		return this.each(function(){
			...
			if(!$t.p.multiselect) {	
				if(pt.className !== "ui-subgrid") {
					...
					if( $t.p.onSelectRow && onsr) { $t.p.onSelectRow.call($t, pt.id, stat, e,reset); }
				}
			} else {
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {		
		...
		mw.className= "ui-widget ui-widget-content ui-corner-all ui-jqdialog ui-dialog";
		mw.id = aIDs.themodal;
		var mh = document.createElement('div');
		mh.className = "ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix";
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal)).html() !== null ) {
				...
				bS  ="<a href='javascript:void(0)' id='"+fid+"_search' class='fm-button ui-state-default ui-corner-all fm-button-icon-right ui-reset btn'><span class='ui-icon ui-icon-search'></span>"+p.Find+"</a>",
