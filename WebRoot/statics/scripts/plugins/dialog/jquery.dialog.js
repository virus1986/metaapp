(function($){
	...
	$.dialog = function(options){
		...
		if(window.dialog_not_render_top){
			options.renderTop=false;
		}
	...
	$._dialog = function(options){
		...
		renderContent.call(this,jQDomEl) ;
	...
	var topWin = window ;
	var $config=topWin.$dialogConfig||{
		okButton:"OK",
		cancelButton: 'Cancel'
	};
	$.dialog.defaults = $._dialog.defaults = {
		okButtonText:$config.okButton,
		cancelButtonText:$config.cancelButton,
		title:"",
		escClose:true,
		okEvent:function(){close.call(this) ;},
	...
	function renderContent(jQDomEl){
		zIndex++ ;
		var me = this ;
		...
		var args = $.extend({},this.settings) ;
		args.data = null ;
		args = $.extend(args , this.settings.data) ;
		
		//set params
		this.settings.window.$_dialogArguments = args ;//this.settings.data ;
		...
		if( this.settings.content ){//文本信息
			...
			var targetDom = this.settings.opener.jQuery(this.settings.contentSelector).html();
			unblock.call(this) ;
			this.frwDom.show();
			...
			var split = url.indexOf("?")!=-1?"&":"?" ;
		 	var options = {
			  	...
			  	success: function(html){
			  		me.isCompleted=true;
			  		unblock.call(me,function(){
				  		if(jQDomEl){
				  			jQDomEl.fadeIn("fast").center(false);
				  		}
	...
	function close(){
		...
		//清除iframe，释放内存
    	setTimeout(function(){
    		$(me.frwDom.find("iframe")).each(function(){
        		try{
        		   	   jQuery.memory.iframe( $(this)[0] ) ;
        		}catch(e){}
        	}) ;
    		me.frwDom.remove() ;
    	},100) ;
	...
	jQuery.fn.center = function(f) {  
	    return this.each(function(){  
	        ...
	        if(!f || f == "vertical") {  
	            //s.top = "0px";  
				if(s.position == 'fixed'){
					s.top=(w.height() - parseInt(jQuery.css(this,"height")))/2 +"px";
				}else{
					if(p.nodeName.toLowerCase() == "body") {  
