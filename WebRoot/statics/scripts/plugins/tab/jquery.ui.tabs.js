...
$.widget("ui.__tabs", {
	options: {
		...
		panelContainer:null,
		spinner: '<em>加载中&#8230;</em>',
		tabTemplate: '<li><a  href="#{href}" tabId="#{tabId}" class="btn  mini"><span>#{label}</span><b class="ui-icon ui-icon-close ui-tab-close"></b></a></li>',
		closeTabTemplate:'<li><a  href="#{href}" tabId="#{tabId}" class="btn  mini"><span>#{label}</span><b class="ui-icon ui-icon-close ui-tab-close"></b></a></li>'
	...
	_create: function() {
		...
		if(this.options.tabs){
			...
			$(this.options.tabs).each( function(){
				...
				var meEle=me.options.panelContainer || me.element;
				if( meEle.find('#'+this.content).get(0) )return ;
				meEle.append( $('#'+this.content).show()  ) ;
			} ) ;
		}
	...
	_cleanup: function() {
		...
		this.lis.filter('.ui-state-processing').removeClass('ui-state-processing')
				...
				.each(function() {
					var el = $(this);
					el.data('label.tabs') && el.html(el.data('label.tabs')).removeData('label.tabs');
				});
	},

	_tabify: function(init) {
		var _list=this.element.children('ol,ul').eq(0);
		this.list=_list;
	...
	load: function(index) {
		...
		if(url.indexOf('$')!=-1){//iframe load
			...
			error: function(xhr, s, e) {
				...
				$(self._sanitizeSelector(a.hash)).html(xhr.responseText);
				// take care of tab labels
				self._cleanup();
    ...
    _updateNav: function() {
        ...
        if (!o.circular) {
        	...
        	if( lastLi.length ){
	            ...
	            prev = parentLeft <= (liFirstLeft+10)  ;//> 0 ;
...
$.fn.tabs = function(json_obj){
	...
	tabsWidget = function(){
		...
		this.getSelectedId = function(){
			var context=this.$.__tabs();
			var panelContainer=this.$.__tabs("option","panelContainer");
			if(panelContainer){
				context=$(panelContainer);
			}
			return context.find(".ui-tabs-panel:visible").attr("id");
		};
