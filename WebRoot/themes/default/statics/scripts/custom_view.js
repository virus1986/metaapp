		...
		createNew:function(_options){
			...
			var ViewCustom={
					...
					initPage:function(){
						...
						//绑定快捷操作
						$(".table-img-icon",__context).each(function(){
							 var me = $(this) ;
							 var id = $(this).attr("id") ;
							 var temp  = id.split("_") ;
							 var btnId = temp[1] ;
							 ...
							 if( ViewCustomTools.getToolbarBtn(toolbarBtns,btnId) ){//有效
							 	me.css({"position":"relative"}).append("<div class='enable-status'>"+
											   "<img  style='position:absolute;right:0px;top:0px;' status='enable' src='"
											   +imageRoot+"/icon_tool_092.gif' ></div>") ;
							 }else{
							 	me.css({"position":"relative"}).append("<div class='enable-status'>"+
											   "<img  style='position:absolute;right:0px;top:0px;' status='disable' src='"
					...
					bindToolbarBtnEvents:function(){
						var imageRoot = ViewGlobal.imageRoot;
						var draggEl = null ;
						$(".table-img-icon",__context).unbind("click").bind("click",function(){
							 var me = $(this).find(">.enable-status") ;
							 var status = $(this).find("img[status]").attr("status") ;
							 if( status == "enable"){
								 me.empty()
								   .prepend("<img  status='disable' style='position:absolute;right:0px;top:0px;' src='"+imageRoot+"/icon_tool_122.gif' >").parent().addClass("ui-state-disabled")  ;
							 }else{
								 me.empty()
								   .prepend("<img  status='enable' style='position:absolute;right:0px;top:0px;' src='"+imageRoot+"/icon_tool_092.gif' >").parent().removeClass("ui-state-disabled")  ;
							 }
				
						 }) ;
						$(".table-img-icon",__context).draggable({
								helper:"clone",
						...
						$(".table-img-icon",__context).droppable({
							accept:".table-img-icon",
					...
					initGrid : function(context){
						...
						$(".table-img-icon",__context).unbind("click") ;
