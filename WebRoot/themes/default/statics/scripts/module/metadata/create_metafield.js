define(["require","./OptionsetManager"],function(require,OptionsetManager){
	return {
		...
		createNew:function(_options){
			...
			Form.init(context,"#createFieldForm");
			/**End 界面显示信息初始化**/
			/**Begin 公共方法定义**/
			...
			function getOptions(optionSetName){
				var optionSet=null;
				var options=null;
			...
			$("[name='optionSetName']", context).click(function(){
			...
			$(".expression select", context).change(function(){
				...
				if(regValue == ""){
					...
					$("#changingColspan").attr("colspan", "2");
				}
			});
...
//		 		data["fieldTypeParams"]=fieldTypeParams;
				var fieldTypeParams = {};
				$('.fieldTypeAttrs input, .fieldTypeAttrs select', context).each(function(index){
					var $t = $(this), name = $t.attr('name'), val = $t.val();
			...
			function resetFieldTypeAttrs(fieldTypeName){
				...
				if(fieldTypeName=="Boolean"){
					...
					$("[name='parentField']",context).change(function(){
						...
						//consolelog(initialOptionSource);
						var parentOptionSource=findOptionSourceByParentField(parentField)||initialOptionSource||"optionSet";
