...
define(["cellPos"],function(){	
	...
	var binWrapperClass="_i-wrapper";
	...
	module.draggable=function(el,options){
    	...
    	function movePlaceHolder(event,ui,options){
    		...
    		if(curDropEl.attr("id")==="i-bin-overlay"||curDropEl.closest("#i-bin-overlay").length>0 || curDropEl.length==0){
    			//偶尔会定位到placeholder自己
    			placeHolder.pl.hide();
    			placeHolder.visible=false;
    			return;
    		...
    		if(placeHolder.direction==="in"&&direction!=="in"){
    			placeHolder.pl.hide();
    			placeHolder.visible=false;
    			return;
    		}
