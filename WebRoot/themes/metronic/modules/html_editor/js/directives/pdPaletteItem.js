	    ...
	    link: function postLink(scope, iElement, iAttrs) {
	    	...
	    	domUtils.draggable($el,{
	    		...
	    		refuse:function(event,ui,curDropEl,direction){
	    			var refuse=false;
	    			if($.isFunction(item.refuse)){
	    				refuse=item.refuse(event,ui,curDropEl,direction);
	    				$(event.target).data("refuse",item.refuse);
	    			}
	    			if(!refuse){
		      	...
		      	stop: function(event,ui,placeholder) {
	    			...
	    			if(itemName) {
	    				...
	    				scope.$apply(function(){
	    					...
	    					$(newAddItemDom).data("refuse",$(event.target).data("refuse"));
