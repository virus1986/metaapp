...
//	   					return profileParamValue == "jbpm";
            ...
            success: function(response) {
    	   		try {
    	   			if(response.responseText && response.responseText.length > 0) {
    	   				//this._createCookie("selectcolortheme", response.responseText, 30);
    	   				
    	   				var themejson = response.responseText.evalJSON();
    	   				var themeobj = themejson["themes"];
    	   				var toapplyobj = themeobj[tname];
    	   				
    	   				ORYX.EDITOR._canvas.getChildShapes().each((function(child) {
	...
	applyThemeToNodes: function(child, themeObj) {
		var childgroup = child.getStencil().originGroups().first();
